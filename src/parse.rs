...
impl_require_feature!(Function {
    recurse: params, ret, body;
});
...
impl_require_feature!(FunctionParam {
    recurse: ty;
});
...
impl_require_feature!(Assignment {
    recurse: pattern, ty, expression;
});
...
impl_require_feature!(TypeAlias {
    recurse: ty;
});
...
impl_require_feature!(Expression {
    recurse: inner;
});
...
impl_require_feature!(SingleExpression {
    recurse: inner;
});
...
impl_require_feature!(Match {
    recurse: scrutinee, left, right;
});
...
pub trait ParseFromStrWithErrors: Sized {
    /// Parse a value from the source, pushing lexical and parse errors to
    /// `handler`.
...
pub(crate) struct CheckedParseContext<'features, 'handler> {
    unstable_features: &'features UnstableFeatures,
    handler: &'handler mut ErrorCollector,
}

impl<'features, 'handler> CheckedParseContext<'features, 'handler> {
    pub(crate) fn new(
        unstable_features: &'features UnstableFeatures,
        handler: &'handler mut ErrorCollector,
    ) -> Self {
    ...
    pub(crate) fn parse_program(
        ...
        let mut local_handler = ErrorCollector::new();
        let program = Program::parse_from_str_with_errors(source.clone(), &mut local_handler);
        let program =
            self.unstable_features
                .validate_parse(program, source.clone(), &mut local_handler);

        if local_handler.has_errors() {
            self.handler.extend_with_handler(source, &local_handler);
            None
        } else {
            program
        }
    ...
    pub(crate) fn handler(&mut self) -> &mut ErrorCollector {
        self.handler
    }
...
mod test {
    ...
    fn parse_with(input: &str, features: &UnstableFeatures) -> (bool, String) {
        ...
        let program = {
            let mut parse_ctx = CheckedParseContext::new(features, &mut handler);
            parse_ctx.parse_program(source)
        };
        (
            program.is_none() || handler.has_errors(),
