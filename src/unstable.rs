...
impl UnstableFeatureManager {
    ...
    pub fn check_program(&self, program: &Program, handler: &mut ErrorCollector) {
        for item in program.items() {
            ...
            let mut check_and_report = |feature| {
                if let Err(error) = self.check_feature(feature) {
                    handler.push(RichError::new(error, *use_decl.span()));
                }
            };

            check_and_report(UnstableFeature::UseKeyword);

            if use_decl
                .drp_name()
                .is_ok_and(|drp| drp == crate::driver::CRATE_STR)
            {
                check_and_report(UnstableFeature::CrateKeyword);
            }
            ...
            if has_alias {
                check_and_report(UnstableFeature::AsKeyword);
            }
        }
    }
