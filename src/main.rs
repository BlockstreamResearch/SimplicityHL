...
impl Target {
    ...
    /// # Safety
    ///
    /// For an external target, every required symbol in the selected library must
    /// have the exact ABI and signature expected by this compiler. Built-in targets
    /// have no additional safety requirements.
    #[allow(clippy::unnecessary_wraps)]
    unsafe fn jet_hinter(&self) -> Result<Box<dyn JetHinter>, Box<dyn std::error::Error>> {
        match self {
            Target::Elements => Ok(Box::new(ElementsJetHinter::new())),
            Target::Core => Ok(Box::new(CoreJetHinter::new())),
            #[cfg(feature = "external-jets")]
            Target::External(path) => {
                // SAFETY: The caller guarantees that every loaded symbol has the exact ABI
                // and signature expected by `init_external_jet_lib`.
                unsafe {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // SAFETY: For an external target, the CLI requires the supplied library to export
    // every external jet symbol with the ABI and signature expected by this compiler.
    // Built-in targets do not load external code.
    let jet_hinter = unsafe { target.jet_hinter()? };
    let template =
        match TemplateAst::new_with_dep(source, &dependencies, &unstable_features, jet_hinter) {
            Ok(program) => program,
            Err(diags) => {
